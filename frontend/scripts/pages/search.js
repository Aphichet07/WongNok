...
function initSearchPage() {
    ...
    document.querySelectorAll(".hero__tags .tag").forEach(tag => {
        tag.addEventListener("click", async () => {
            ...
            const searchInput = document.getElementById("hero-search-input");
            if(searchInput) searchInput.value = filterType || tag.textContent.replace('#', '');

            await performSearch(() => shopService.searchByName(searchInput.value));
            document.getElementById("filters").scrollIntoView({ behavior: "smooth" });
        });
    });
}
    ...
    const createBtn = (text, targetPage, isActive = false, isDisabled = false) => {
        ...
        if (!isDisabled) {
            btn.addEventListener("click", () => {
                ...
                document.getElementById("shop-result").scrollIntoView({ behavior: "smooth", block: "start" });
            });
        }
        return btn;
    };

    container.appendChild(createBtn("‹", CURRENT_PAGE - 1, false, CURRENT_PAGE === 1));

    for (let i = 1; i <= totalPages; i++) {
        container.appendChild(createBtn(i, i, i === CURRENT_PAGE));
    }

    container.appendChild(createBtn("›", CURRENT_PAGE + 1, false, CURRENT_PAGE === totalPages));
}